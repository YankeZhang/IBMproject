...
function matches(Staff: Staff, term: string, pipe: PipeTransform) {
  return Staff.name.toLowerCase().includes(term)
    ...
    || pipe.transform(Staff.lecture).includes(term)
    || pipe.transform(Staff.support).includes(term)
    || pipe.transform(Staff.hachathon).includes(term)
    || pipe.transform(Staff.other).includes(term)
...
export class StaffService {
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 1. sort
    let staffs = sort(STAFFS, sortColumn, sortDirection);
